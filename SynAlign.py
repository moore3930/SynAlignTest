...
class SynAlign(Model):
    ...
    def load_data(self):
        ...
        def load_dataset(path, num_examples=None):
            ...
            tensor = self.source_tokenizer.texts_to_sequences(inp_lang)
            input_tensor = tf.keras.preprocessing.sequence.pad_sequences(tensor, padding='post')

            tensor = self.target_tokenizer.texts_to_sequences(targ_lang)
        ...
        num_examples = 50000
        source_tensor, target_tensor = load_dataset(self.path_to_file, num_examples)

        # Creating training and validation sets using an 80-20 split
        source_tensor_train, source_tensor_val, target_tensor_train, target_tensor_val = train_test_split(
        ...
        print(len(self.vocab_source_freq))

        # dataset = tf.data.Dataset.from_tensor_slices(
        #     (source_tensor_train, target_tensor_train))
        dataset = tf.data.Dataset.from_tensor_slices(
            (source_tensor_train, target_tensor_train)).shuffle(BUFFER_SIZE)
    ...
    def add_model(self):
        ...
        self.source_sent, self.target_sent = iter.get_next()
        ...
        ta_soft_score = tf.nn.softmax(ta_score)
    ...
    def __init__(self, params):
        ...
        # data file
        # self.path_to_file = "./data/en-de.txt"
